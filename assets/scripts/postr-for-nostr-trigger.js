...
[...postr_for_nostr_trigger_elements].map((element) => {
    element.addEventListener('click', function () {
        ...
        window.open(postr_for_nostr_trigger_localize.site_url + '?action=postr-for-nostr&post_id=' + id + '&post_type=' + type, 'targetWindow', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=700');
        return false;
    });
});
